...
document.getElementById("bank-button").addEventListener("click", bankFunction);
...
document.getElementById("computers").onchange = function () {
  ...
  const selectedComputer = computerArray.filter(
    (computerArray) => computerArray.title == selectedvalue
  );

  //Sets ids from html to the data for selected computer
  document.getElementById("description-computer").innerText =
    "Description: " + selectedComputer[0].description;
  document.getElementById("price-computer").innerText =
    selectedComputer[0].price;
