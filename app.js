document.getElementById("loan-button").onclick = function() {
  ...
  if (document.getElementById("loan-balance").innerHTML !=0){ alert("You cannot take a loan while you´re still paying off one")
  }else{
  
  
      let loanPromt = prompt("How much would you like to loan?");
     if (Number(loanPromt) > Number(document.getElementById("bank-balance").innerHTML)*2) {
          alert("You cannot loan more than double you bank balance")
        }else{
          alert("You've got a loan")
          document.getElementById("loan-balance").innerHTML =Number(loanPromt)
  ...
  fetch("https://noroff-komputer-store-api.herokuapp.com/computers")
   ...
   document.getElementById("test-button").onclick = function() {console.log(Number(document.getElementById("price-computer").innerText))}
